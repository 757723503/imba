    ...
    private async _NpcSpawned(data: GameEventProvidedProperties & NpcSpawnedEvent): Promise<void> {
        ...
        if (Is_CDOTA_BaseNPC(entity) && entity.IsBaseNPC() && data.is_respawn == 0) {
            ...
            if (entity.AddAbility != null) {
                const _ability_custom_base_attack = entity.AddAbility('imba_custom_base_attack');
                entity._ability_custom_base_attack = _ability_custom_base_attack;
                _ability_custom_base_attack?.SetAbilityIndex(30);
